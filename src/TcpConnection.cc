...
void TcpConnection::sendInLoop(const Message &msg)
{
    ...
    if(_loop)
    {
        //EventLoop本身没有发送数据的能力，所以需要借助TcpConnection
        //的send方法进行发送，就打包成一个任务
...
{
    Message mess;
    _sockIO.readn(&mess, sizeof(mess));
...
InetAddress TcpConnection::getLocalAddr()
{
    ...
    int ret = getsockname(_sock.fd(), (struct sockaddr *)&addr, &len);//获取成功连接的客户端套接字具体信息
    if(-1 == ret)
    {
        perror("getsockname");
    }
